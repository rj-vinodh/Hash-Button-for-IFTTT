...
function switchView(){
	if($("#options").attr("style")=="")
	{
		$("#options").attr("style", "display:none");
		$("#frontPage").attr("style", "")
...
function addButton(name,values,table){
	...
	if(name!=""){
	  ...
	  e.value = values;
	 e.name=name;
	   e.className="button  button-rounded button button-3d";
	   e.innerHTML='<i class="material-icons m-t-5">edit</i>';
	  var cellEdit = $("<td />");
	  cellEdit.attr('id', "editTd");
	  console.log(cellEdit);
	  cellEdit.append(e);
	  row.append(cellEdit);
	  var db=d.createElement('button');
	  db.id ="deleteButton";
	  db.value = values;
	   db.className="button  button-rounded button button-3d";
       db.name=name;
	   db.innerHTML='<i class="material-icons m-t-5">delete</i>';
	  var cellDelete = $("<td />");
	  cellDelete.attr('id', "deleteTd");
...
$("body").append(diShowAlert);
diShowAlert.append(innerDiv);
innerDiv.attr( "style","position: absolute;top: 20%;left: 10%;bottom: 20%;right: 10%;background: white");
innerDiv.html(insideAlert||"");

}

var ij=0;

function updateStatus(sta,name){
	/*limiting maximum no of elements to be stored in local storage to 25 for
	reducing memory usage */
	if(ij>25)
		fired.shift();
			fired.push(name);
			var len=fired.length
			var ord=len-1;
...
function addListners(){
	var ad=$("[id='actionButton']");
	for(var i=0;i<ad.length;i++){
ad[i].addEventListener("click", function(event){
	var butc=$(this);
	var im=$('<img id="im" class="hello" style="-webkit-user-select: none" src="ring.gif">');
	$(this).append(im);
